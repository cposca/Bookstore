...
public class SignupServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (!(request.getParameter("success") == null)) {
			...
			List<LoginBean> list;
			if (username.isEmpty() || password.isEmpty() || fName.isEmpty() || lName.isEmpty() || street.isEmpty()
					|| country.isEmpty() || province.isEmpty() || zip.isEmpty() || phone.isEmpty()) {
					...
					if (list.isEmpty()) {
						if (password.equals(repeatPassword)) {
							if (password.length() >= 6) {
								...
								String encryptedPass = hashToString(
										sha256(password, DatatypeConverter.printBase64Binary(salt)));
